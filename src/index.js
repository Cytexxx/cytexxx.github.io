import { Axis, Space, SceneLoader, Color3, Engine, Scene, ArcRotateCamera, Vector3, HemisphericLight, PointLight, Mesh, MeshBuilder, Animation, DynamicTexture, StandardMaterial } from "@babylonjs/core";
...
async function initBabylonJS(){
    ...
    var greyColor = new Color3(.5,.5,.5);
    var color1 = new Color3(0, 0.4392, 0.006);
    var color2 = new Color3(0.0274, 0.2745, 0.4666);
    var color1hell = new Color3(color1.r*2, color1.g*2, color1.b*2);
    var color2hell = new Color3(color2.r*2, color2.g*2, color2.b*2);

    var createScene = async () => {
        var scene = new Scene(engine);
        scene.clearColor = new Color3(.2, .2, .2);
        scene.fogMode = Scene.FOGMODE_LINEAR;
        scene.fogStart = 20;
        scene.fogEnd = 25;
        scene.fogColor = new Color3(.2, .2, .2);

        var camera = new ArcRotateCamera("Camera", Math.PI / 2, Math.PI / 2, 2, new Vector3(0, 0, 6), scene);
        
                 
        if(canvas.width < canvas.height){ 
            scene.activeCamera.fovMode = 1;
        }else{            
        ...
        var light1_mirror = new HemisphericLight("light1_mirror", new Vector3(0, 1, 0), scene);
        light1_mirror.intensity = .3;
        light1_mirror.diffuse = greyColor;

        var light2 = new PointLight("light2", new Vector3(0, 0, -20), scene);
    ...
    window.addEventListener("resize", function () {                
        if(canvas.width < canvas.height){ 
            scene.activeCamera.fovMode = FOVMODE_HORIZONTAL_FIXED;
        }else{            
            scene.activeCamera.fovMode = 0;
        }
        engine.resize();
...
function createImagePlane(scene){
    let plane = MeshBuilder.CreatePlane("plane",{size:10}, scene);
    plane.position = new Vector3(0, 0, 5);
    plane.visibility = 0.75;

    let planeMaterial = new StandardMaterial("planeMat", scene);
    planeMaterial.diffuseTexture = new Texture("src/grad.png");
    planeMaterial.backFaceCulling = false;
    planeMaterial.alphaMode = Engine.ALPHA_ADD;
    
    plane.material = planeMaterial;
}

function loadMesh(scene){
    SceneLoader.LoadAssetContainer("./src/models/", "tube.glb", scene, function (container) {
        ...
        meshes[0].position = new Vector3(0,0,50);
        var materials = container.materials;
        for (let i = 0; i < materials.length; i++) {
            const material = materials[i];
            material.backFaceCulling = false;
            material._albedoColor = new Color3(.5,.5,.5);
        }
...
function runAnimation(mesh, scene){
    ...
    for (let i = 0; i < 4; i++) {
        ...
        switch (i) {
            case 0:
                clone.position = new Vector3(0,0,-24.48);
            break;
            case 1:
                clone.position = new Vector3(0,0,-16.32);
            break;
            case 2:
                clone.position = new Vector3(0,0,-8.16);
            break;
            case 3:
    ...
    scene.registerBeforeRender(function(){
        ...
        for (let i = 0; i < meshes.length; i++) {
            const mesh = meshes[i];
            mesh.translate(Axis.Z, speed*deltaTime /100, Space.WORLD);
...
class GradientForeground {
    ...
    (scene) => {
        ...
        let dynTexture = new DynamicTexture("diffuseForeground", 512, scene, true);

        let planeMaterial = new StandardMaterial("planeMat", scene);
        planeMaterial.diffuseTexture = dynTexture;
        planeMaterial.emissiveColor = new Color3(1,1,1);
        planeMaterial.backFaceCulling = false;
        planeMaterial.alphaMode = Engine.ALPHA_MULTIPLY;
        ...
        let animation = new Animation("Animation", "material.diffuseTexture.vOffset", 30, Animation.ANIMATIONTYPE_FLOAT, Animation.ANIMATIONLOOPMODE_CYCLE);
        ...
        scene.registerBeforeRender(() => {
            //plane.rotate(Axis.Y, rot, Space.WORLD);
        })
    }
}